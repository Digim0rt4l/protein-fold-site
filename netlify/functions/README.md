...
- **Storage**: a Netlify Function reads and writes one shared JSON state file in a
  ...
  `get-status` keeps a short-lived in-memory cache (a few seconds) so many browser
  tabs polling at once don't turn into that many separate GitHub API reads; this
  can make the displayed state up to a few seconds behind the true latest write.
