...
real force field: no side chains, no solvent, no validated all-atom physics. It will not
produce new, publishable Alzheimer's science on its own.
...
- **Backbone construction**: each residue contributes an N, C-alpha, and C atom, placed
  ...
  assumed trans.
- **Degrees of freedom**: the only things that move are each residue's phi and psi
  torsion angles. Bonds are geometrically exact by construction, not softly enforced.
- **Starting point**: a randomized, extended chain, not the known answer. The real
  ...
  its way there.
- **Scoring function**: a steric clash term between backbone atoms that aren't close
  neighbors in the chain, plus a soft torsional preference pulling phi/psi in the known
  helical regions toward the classic alpha-helix values.
...
- **Different protein**: edit `data/protein.json` (sequence, helix ranges, citation).
  Geometry and scoring derive from it automatically; only the residue count and helix
  ranges are actually used by the model.
...
- No side chains, no solvent, no validated all-atom force field; the scoring function
  is a deliberately simplified clash-plus-torsion-preference model.
